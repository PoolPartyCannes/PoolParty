...
contract UniswapV3ComposerTest is TestHelperOz5 {
    ...
    function setUp() public virtual override {
        ...
        bSwapRouter = new SwapRouterMock(
            address(bTokenIn),
            address(bTokenOut),
            swapAmountOut
        );
    ...
    function test_lzCompose() public {
        ...
        assertEq(
            bSwapRouter.lastSender(),
            address(bComposer),
            "SwapRouter sender mismatch"
        );
        assertEq(
            bSwapRouter.lastTokenIn(),
            address(bTokenIn),
            "TokenIn address mismatch"
        );
        assertEq(
            bSwapRouter.lastTokenOut(),
            address(bTokenOut),
            "TokenOut address mismatch"
        );
        assertEq(bSwapRouter.lastFee(), fee, "Fee tier mismatch");
        assertEq(
            bSwapRouter.lastRecipient(),
            receiver,
            "Recipient address mismatch"
        );
        assertEq(bSwapRouter.lastAmountIn(), swapAmountIn, "AmountIn mismatch");
        assertEq(
            bSwapRouter.lastAmountOut(),
            swapAmountOut,
            "AmountOut mismatch"
        );
        ...
        assertEq(
            bTokenOut.balanceOf(receiver),
            swapAmountOut,
            "Receiver TokenOut balance incorrect"
        );
    }
    ...
    function test_lzCompose_Fallback() public {
        ...
        uint256 initialComposerBalance = IERC20(bOFT.token()).balanceOf(
            address(bComposer)
        );
        uint256 initialReceiverBalance = IERC20(bOFT.token()).balanceOf(
            receiver
        );
